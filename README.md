...
## Usage
	...
	var res = madge('path/to/app.js');
	console.log(res.obj());

## madge(filename, config)

* `filename` (string) The file to be parsed for dependencies
* `config` (object) Optional [configuration](#configuration)
...
`showFileExtension` | Boolean | false | If file extensions should be visible
...
This config should be a JSON formatted file placed in `.madgerc` in your project or home folder. See [configuration](#configuration) for valid settings. Look [here](https://github.com/dominictarr/rc#standards) for alternative locations for the file.
